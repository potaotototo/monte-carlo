...
Defaults are 64 GiB total storage, 2,000,100 files, 64 MiB retained free space, and 128 MiB per manifest. Checkpoint cadence is automatic by default: at least 64 newly accepted blocks per checkpoint and, for large runs, a cadence chosen to produce no more than 1,024 periodic full snapshots. This bounds the default full-snapshot write amplification while retaining simple recovery. An explicit nonzero `--checkpoint-blocks` value overrides the calculation and intentionally accepts the corresponding recovery-granularity/write-cost tradeoff.
