...
Full manifests remain an intentional R2 simplicity decision, but the default cadence is now adaptive: it is never more frequent than every 64 blocks and targets at most 1,024 periodic snapshots per run. Explicitly requesting a smaller interval is allowed because recovery granularity may be more valuable than write cost for a particular workload. Manifest CRC32C is table-driven, serialization reserves its expected capacity, and checkpointing no longer copies the full optional-result vector.
