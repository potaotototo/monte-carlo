...
CHECKPOINT_GATE_BENCHMARK := $(BUILD_DIR)/benchmark_checkpoint_gate
...
$(TESTS): $(LIB_SOURCES) tools/checkpoint_gate_stats.cpp tests/test_main.cpp $(HEADERS) $(INTERNAL_HEADERS) $(TOOL_HEADERS) $(BUILD_CONFIG_STAMP) | $(BUILD_DIR)
	$(CXX) $(MC_CPPFLAGS) $(CXXFLAGS) $(filter %.cpp,$^) $(LDFLAGS) $(LDLIBS) -o $@
...
benchmark-tools: $(BENCHMARK) $(AGGREGATION_BENCHMARK) $(PERSISTENCE_BENCHMARK) $(CHECKPOINT_GATE_BENCHMARK)
...
$(CHECKPOINT_GATE_BENCHMARK): $(LIB_SOURCES) tools/checkpoint_gate_stats.cpp tools/benchmark_checkpoint_gate.cpp $(HEADERS) $(INTERNAL_HEADERS) $(TOOL_HEADERS) $(BUILD_CONFIG_STAMP) | $(BUILD_DIR)
	$(CXX) $(MC_CPPFLAGS) $(CXXFLAGS) $(filter %.cpp,$^) $(LDFLAGS) $(LDLIBS) -o $@
...
clean:
	rm -f $(RUNNER) $(TESTS) $(BENCHMARK) $(AGGREGATION_BENCHMARK) $(PERSISTENCE_BENCHMARK) $(CHECKPOINT_GATE_BENCHMARK) $(REPLAY_FAILURE) $(CRASH_MATRIX) $(RNG_STREAM) $(BUILD_CONFIG_STAMP) $(BUILD_CONFIG_STAMP).tmp
